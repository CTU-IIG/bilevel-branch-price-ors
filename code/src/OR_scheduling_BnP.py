...
class BnP_Problem(Problem):
    def __init__(self, instance: str, parameters: str, name: Optional[str] = None):
        ...
        assert self.decentralized is False
        self.logs = []
        ...
        self.best_integral_solution: float = 1000
    ...
    def master_problem(self, a: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]], vtype: Any = GRB.CONTINUOUS, cutoff: float = None):
        """
        ...
        model_mp.setObjective(
            self.alpha * (self.capacity - gp.quicksum(
    ...
    def subproblem(self, model_mp: gp.Model, a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]], branching_history: List):
        """
        ...
        a, g, delta, w = self._deepcopy(a, g, delta, w)
        ...
        for s in range(self.num_surgeons):
            ...
            if model_surgeon.Status == GRB.OPTIMAL and model_surgeon.objVal + mus[s] > self.epsilon:
                ...
                if not self.multiple_patterns_per_iteration:
                    ...
                    return columns_generated, a, g, delta, w

        self.subproblem_time += time.time() - start_time
        return columns_generated, a, g, delta, w

    def column_generation(self, a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]], branching_history: List):
        """
        ...
        while columns_generated:
            ...
            model_mp = self.master_problem(a=a, delta=delta, w=w, vtype=GRB.CONTINUOUS)

            if self.subproblem_time + self.mp_time > self.timeout or time.time() >= self.stop_time:
                return self._on_time_over()

            columns_generated, a, g, delta, w = self.subproblem(
                model_mp=model_mp, a=a, g=g,
                delta=delta, w=w,
        ...
        return model_mp, a, g, delta, w, num_columns_added, num_CG_iterations

    def branch(self, a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]], branching_history: List, depth: int, is_rightmost: bool, bound: float = 0.):
        """
        ...
        if not column_generation_result:
            return
        model_mp, a, g, delta, w, num_columns_added, num_CG_iterations = column_generation_result
        ...
        model_heuristic = self.master_problem(a=self.a, delta=self.delta, w=self.w, vtype=GRB.INTEGER, cutoff=self.best_integral_solution - 1 + self.epsilon)
        ...
        branching_s, branching_b, branch_left, branch_right = self._get_most_fractional_branch(model_mp, a, g, delta, w)
        ...
        if add_dummy_pattern:
            a, g, delta, w = self._add_dummy_pattern(dummy_pattern, branching_s, a, g, delta, w)

        self.graph.add_edge(start=current_node, end=self.num_nodes, s=branching_s, b=branching_b, value=1)
        self.branch(
            a=a, g=g, delta=delta, w=w,
    ...
    def _get_subproblem_model(self, s: int, lambdas: dict, branching_history: List[dict]) -> gp.Model:
        ...
        model_surgeon.setObjective(
            ...
            ) - self.beta * w_expression + self.alpha * delta_expression,
            GRB.MAXIMIZE
        )
    ...
    def _get_most_fractional_branch(self, model_mp: gp.Model, a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]]) -> [int, int, dict[str, List[Union[np.ndarray, int]]]]:
        ...
        for pattern, assigned_patients, delta_, w_ in zip(a[branching_s], g[branching_s], delta[branching_s], w[branching_s]):
        ...
        return (
            branching_s,
            branching_b,
            {"a": new_a_for_left, "g": new_g_for_left, "delta": new_delta_for_left, "w": new_w_for_left},
            {"a": new_a_for_right, "g": new_g_for_right, "delta": new_delta_for_right, "w": new_w_for_right},
        )

    def _add_dummy_pattern(self, dummy_pattern: np.ndarray, branching_s: int, a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]]):
        # --- MODEL ---
        ...
        for s in range(self.num_surgeons):
            for k in range(len(a[s])):
                if round(model.getVarByName(f'theta[{s},{k}]').x) == 1:
                    ...
                    self.best_patients_in_block[s] = []
                    for block in self.best_blocks[s]:
                        assigned_patients = list(np.where(g[s][k][:, block.id] == 1)[0])
                        self.best_patients_in_block[s].append((block, [self.patients[p] for p in assigned_patients]))
                    break
    ...
    @staticmethod
    def _deepcopy(a: List[List[np.ndarray]], g: List[List[np.ndarray]], delta: List[List[int]], w: List[List[int]]):
        a = copy.deepcopy(a)
        g = copy.deepcopy(g)
    ...
    def get_result(self) -> ProblemResult:
        ...
        result.obj = self.best_integral_solution
    ...
    def run(self):
        ...
        self.branch(a=self.a, g=self.g, delta=self.delta, w=self.w, branching_history=[], depth=0, is_rightmost=True)
...
if __name__ == "__main__":
    problem = BnP_Problem("50_ordays_5_load_1.5_surgeons_12", "s5_a1_b1_t1200_InH0_MuP1_LCR1_LC2")
    problem.run()
